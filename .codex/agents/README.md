...
- [`rules-fixer.toml`](rules-fixer.toml) — resolves rules-checker findings by editing governance documents, instruction files, and harness prompts, never settling a contradiction or changing what a rule requires. Use it between checker runs inside the rules-quality-gate loop.
