...
SWE Grilling is a personal interview-preparation workspace. Favor hands-on problem solving: complete coding drills yourself before using tools for review, formatting, or feedback. Keep each exercise focused on one skill and record reasoning when useful.
...
Keep implementation and tests together under `src/`; use lowercase-hyphenated project directories. Add assets only within the project that needs them.
...
- `npm run run:dummy` builds and runs the demonstration CLI.
...
- `npm run check:markdown-links` validates every repository-local Markdown link and runs on every pre-push. See the [commit hook policy](repo-governance/development/commit-hook-policy.md).
