...
Claude Code exposes `AskUserQuestion`, and this skill uses it whenever the session is interactive. Put the Recommended option first, keep each header at twelve characters or fewer, and leave the free-text entry to the harness, which supplies it. The [harness binding](../../../repo-governance/conventions/grilling-harness-binding.md) holds the shaping details and the Markdown fallback for a non-interactive session.
